(() => {
  ...
  function render() {
    ...
    $('civ-list').innerHTML=filtered.length?filtered.map(c=>`<button class="civ ${c.id===selected?'active':''}" data-civ="${escape(c.id)}" aria-pressed="${c.id===selected}"><span class="seal" aria-hidden="true">${escape(c.seal)}</span><span class="civ-name">${escape(c.name)}<small>${escape(c.en)}</small></span><span class="chevron" aria-hidden="true">›</span></button>`).join(''):'<p class="empty">該当する文明がありません</p>';
  ...
  function renderDetail(c) {
    ...
    $('detail').innerHTML=`<div class="detail-head"><span class="seal" aria-hidden="true">${escape(c.seal)}</span><div><div class="en">${escape(c.en)}</div><h2>${escape(c.name)}</h2></div><span class="badge">${c.edition==='base'?'基本文明':'追加・派生文明'}</span></div>
      <p class="summary">${escape(c.summary)}</p><div class="tags">${c.tags.map(t=>`<span class="tag">${escape(t)}</span>`).join('')}</div>
      <section class="section"><div class="section-title"><h3><span class="number">01</span>文明の強み</h3><small>主な要素をピックアップ</small></div><div class="unique-grid"><div class="info-card"><div class="label">UNIQUE UNITS / 固有・特徴的なユニット</div><dl>${definitions(c.units)}</dl></div><div class="info-card"><div class="label">TECH & SYSTEM / 研究・文明システム</div><dl>${definitions(c.systems)}</dl></div></div></section>
      ...
      <div class="sources"><a href="${escape(c.source)}" target="_blank" rel="noopener noreferrer">${escape(c.en)} 公式文明紹介 ↗</a><p>文明の特徴：公式紹介を要約（確認日 ${c.reviewed}）。紹介ページが現行の数値を反映していない場合があります。戦術・時間・内政：本サイトの編集案。最新パッチでの実戦検証は未実施です。</p></div>`;
  }
